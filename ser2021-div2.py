...
url = 'https://seusa21d2.kattis.com/contests/seusa21d2/standings'
...
for row in rows:
    standing = {
        'contestant_id': row.select_one('td:nth-child(1)').text.strip(),
        'contestant_name': row.select_one('td:nth-child(2)').text.strip(),
        'solve': int(row.select_one('td:nth-child(4)').text.strip()),
        'penalty': int(row.select_one('td:nth-child(5)').text.strip()),
    }

    judges = []
    for idx, td in enumerate(row.select('td')[5:]):
        judge = {
...
# save to `ser2021-div2.json`
with open('ser2021-div2.json', 'w') as f:
